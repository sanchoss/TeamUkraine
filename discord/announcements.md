# 💬 Discord — Team Ukraine

---
## 🇺🇦 Українська

### ℹ️ Інформація про сервер
| Поле | Значення |
|------|----------|
| Посилання-запрошення | `TODO` |
| Канали | #загальне, #гонки, #результати, #тренування |

### 📣 Шаблони оголошень

#### 📝 Щотижнева реєстрація на гонку
```
🇺🇦 **Team Ukraine — [СЕРІЯ] [Етап/Тижд #]**
📅 Дата: 
⏰ Час: 
🗺️ Маршрут: 
🌍 Світ: 
📏 Дистанція:  км /  м набору

**Реєстрація:**
Реагуйте ✅ якщо їдете, ❌ якщо недоступні
Кат A: 
Кат B: 
Кат C: 
Кат D: 

**План гонки:** [посилання або короткі нотатки]
```

#### 🏁 Результати після гонки
```
🏁 **Результати — [СЕРІЯ] [Етап #]**
📅 Дата: 

**Командний фініш:**
🏆 Позиція: 
⏱️ Час: 

**Індивідуальні результати:**
| Гонщик | Поз | Час | Вт/кг |
|--------|-----|-----|-------|
| | | | |

Молодці, команда! 🇺🇦
```

#### 📢 Оголошення нової серії
```
📢 **Нова серія: [НАЗВА СЕРІЇ]**
📅 Дати: 
🔗 Інфо: 
📝 Дедлайн реєстрації: 

Хто бере участь? Реагуйте нижче!
```

---
## 🇬🇧 English

### ℹ️ Server Info
| Field | Value |
|-------|-------|
| Invite Link | `TODO` |
| Channels | #general, #race-events, #results, #training |

### 📣 Announcement Templates

#### 📝 Weekly Race Signup
```
🇺🇦 **Team Ukraine — [SERIES] [Stage/Week #]**
📅 Date: 
⏰ Time: 
🗺️ Route: 
🌍 World: 
📏 Distance:  km /  m elev

**Signup:**
React ✅ if racing, ❌ if unavailable
Cat A: 
Cat B: 
Cat C: 
Cat D: 

**Race Plan:** [link to event file or brief notes]
```

#### 🏁 Post-Race Results
```
🏁 **Results — [SERIES] [Stage #]**
📅 Date: 

**Team Finish:**
🏆 Position: 
⏱️ Time: 

**Individual Results:**
| Rider | Pos | Time | W/kg |
|-------|-----|------|------|
| | | | |

Well done team! 🇺🇦
```

#### 📢 New Event Series Announcement
```
📢 **New Series: [SERIES NAME]**
📅 Dates: 
🔗 Info: 
📝 Signup deadline: 

Who's in? React below!
```
